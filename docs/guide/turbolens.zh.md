...
用户决策与审查元数据在**多次重新扫描后依然持久**：

### 将发现提升到风险登记簿

当关联的风险后续达到 `mitigated`、`monitoring`、`closed` 或 `accepted`（或被删除）时，反向传播引擎会自动把每条关联的合规发现迁移到对应状态（`mitigated`、`verified`、`accepted`，或回到 `in_review`）。风险上记录的接受理由会镜像到发现的审查备注，以确保审计线索保持一致。

### 合规网格上的批量操作

当被授予 `security_compliance.manage` 时，合规网格会显示具备过滤感知的多选能力。勾选表头复选框可选中匹配当前过滤条件的所有行，然后通过粘性工具栏：

- **编辑决定** —— 将所选每条发现批量过渡到所选状态（例如在范围复核之后将一批发现标记为 `not_applicable`）。非法过渡会在部分成功汇总中按行报告，而不是让整个批量失败。
- **删除** —— 永久移除发现（用于在禁用某项法规后清理其遗留发现）。

晋升到风险仍然是单行操作 —— 出于保留每条发现的上下文捕获，刻意不提供批量晋升。
...
**合规发现**（在 v1.11.0 中重新设计）

```
new → in_review → mitigated → verified
                      ↘ accepted          （侧分支，需要理由）
                      ↘ not_applicable    （侧分支，范围复核）
                      ↘ risk_tracked      （晋升为风险时自动设置）
```

`risk_tracked` 永远不会被手动设置 —— 当你在某发现上点击**创建风险**时会自动写入，并在关联风险关闭时由反向传播引擎清除（参见上文的*将发现提升到风险登记簿*）。

对于完整治理工作流（负责人、残余评估、接受理由、Todo 与通知），请将发现提升为风险 —— 完整生命周期位于[风险登记簿](risks.md)。
