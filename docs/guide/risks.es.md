...
El botón **Importar**, junto a «Exportar», carga riesgos de forma masiva desde un archivo `.xlsx`. Haga clic en **Descargar plantilla** para obtener un libro inicial con los encabezados correctos, rellene un riesgo por fila y súbalo. Cada fila crea un riesgo **completamente nuevo** con una referencia `R-NNNNNN` generada automáticamente — cualquier columna de referencia del archivo se ignora (el importador nunca actualiza riesgos existentes).
