...
### Находки переживают повторные сканирования

Решения пользователя и метаданные рецензии **устойчивы между повторными сканированиями**:

### Продвинуть находку в реестр рисков

Когда связанный Риск позднее достигает `mitigated`, `monitoring`, `closed` или `accepted` (или удаляется), движок обратного распространения автоматически переводит каждую связанную находку соответствия в соответствующее состояние (`mitigated`, `verified`, `accepted` или обратно в `in_review`). Обоснование принятия, зафиксированное на Риске, копируется в заметку рецензии находки, чтобы аудиторский след оставался согласованным.

### Массовые действия на сетке «Соответствие»

При наличии `security_compliance.manage` сетка «Соответствие» предлагает множественный выбор, учитывающий фильтры. Поставьте флажок в шапке, чтобы выбрать все строки, соответствующие активным фильтрам, затем используйте закреплённую панель инструментов:

- **Изменить решение** — переведите все выбранные находки в выбранный статус пакетом (например, отметить пачку находок как `not_applicable` после пересмотра охвата). Недопустимые переходы сообщаются построчно в сводке частичного успеха, а не валят весь батч.
- **Удалить** — окончательно удалить находки (полезно для уборки находок регламента, который вы с тех пор отключили).

Продвижение в Риск остаётся построчным действием — массовое продвижение намеренно не предлагается, чтобы сохранить захват контекста по каждой находке.
...
open → acknowledged → in progress → mitigated
                                  ↘ accepted (формальное принятие риска)
                                  ↘ reopen → open
```

**Находки соответствия** (переработаны в v1.11.0)

```
new → in_review → mitigated → verified
                      ↘ accepted          (боковая ветвь, требуется обоснование)
                      ↘ not_applicable    (боковая ветвь, пересмотр охвата)
                      ↘ risk_tracked      (устанавливается автоматически при продвижении в Риск)
```

`risk_tracked` никогда не задаётся вручную — он автоматически записывается, когда вы нажимаете **Создать риск** на находке, и сбрасывается движком обратного распространения при закрытии связанного Риска (см. *Продвинуть находку в реестр рисков* выше).

Для полного governance-процесса (владение, остаточная оценка, обоснование принятия, Todos и уведомления) продвиньте находку в Риск — полный жизненный цикл живёт в [Реестре рисков](risks.md).
