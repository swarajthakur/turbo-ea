...
Il pulsante **Importa** accanto a «Esporta» carica rischi in blocco da un file `.xlsx`. Fai clic su **Scarica modello** per ottenere una cartella di lavoro iniziale con le intestazioni corrette, inserisci un rischio per riga e caricala. Ogni riga crea un rischio **completamente nuovo** con un riferimento `R-NNNNNN` generato automaticamente — qualsiasi colonna di riferimento nel file viene ignorata (l'importatore non aggiorna mai i rischi esistenti).
